...
# Load mô hình một lần khi khởi động app
model = None
try:
...
def index():
    if request.method == 'POST':
        ...
        if not url:
            return render_template('index.html', error="URL không được để trống!")
        ...
        features_list = [(key, value) for key, value in features_dict.items()]
        warning = ""
        if prediction == 1:
            warning = "⚠️  CẢNH BÁO: URL này có nguy cơ cao là phishing! Không nhập thông tin cá nhân."
        else:
            warning = "✅ URL có vẻ an toàn."
        
        return render_template('index.html',
                               url=url,
                               features=features_list,
                               label=label,
                               prob_benign=f"{prob_benign:.2%}",
...
if __name__ == '__main__':
    if model is None:
        print("❌ Không thể khởi động app vì lỗi load model.")
    else:
        app.run(debug=True)